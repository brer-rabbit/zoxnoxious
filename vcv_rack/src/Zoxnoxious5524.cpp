...
Zoxnoxious5524::Zoxnoxious5524() : output1NameString(invalidCardOutputName),
...
void Zoxnoxious5524::pullSamples(const rack::engine::Module::ProcessArgs &args, dsp::Frame<maxAudioChannels> &sharedFrame, int offset) {
...
bool Zoxnoxious5524::pullMidi(const rack::engine::Module::ProcessArgs &args, uint32_t clockDivision, int midiChannel, midi::Message &midiMessage) {
...
void Zoxnoxious5524::onReset(const ResetEvent& e) {
  Module::onReset(e);
  pwLimit = 0;
}

void Zoxnoxious5524::onAttach() {
...
json_t* Zoxnoxious5524::dataToJson() {
  json_t* rootJ = json_object();
  json_object_set_new(rootJ, "pwLimit", json_integer(pwLimit));
  return rootJ;
}

void Zoxnoxious5524::dataFromJson(json_t* rootJ) {
...
CouplingDisplayState Zoxnoxious5524::getCouplingDisplayState() {
  CouplingDisplayState couplingState;

  // bizlogic to identify what is modulating what
  couplingState.vco2RawToVco1Tzfm = params[VCO_TWO_TO_FREQ_VCO_ONE_BUTTON_PARAM].getValue() > 0.f ?
    (inputs[VCO_TWO_MOD_AMOUNT_INPUT].getVoltage() / 10.f) + params[VCO_TWO_MOD_AMOUNT_KNOB_PARAM].getValue() : 0.f;

  couplingState.vco2ShapedToVco1Tzfm = (inputs[VCO_TWO_WAVESHAPE_TZFM_INPUT].getVoltage() / 10.f) +
    params[VCO_TWO_WAVESHAPE_TZFM_KNOB_PARAM].getValue();

  couplingState.vco2RawWaveMask =
    (params[VCO_TWO_WAVE_PULSE_BUTTON_PARAM].getValue() ? RAW_PULSE : 0) |
    (params[VCO_TWO_WAVE_SAW_BUTTON_PARAM].getValue() ? RAW_SAW : 0) |
    (params[VCO_TWO_WAVE_TRI_BUTTON_PARAM].getValue() ? RAW_TRI : 0);
  couplingState.vco2ShapedWaveMask = couplingState.vco2RawWaveMask;

  couplingState.vco1ToVco2ExpFm = params[VCO_ONE_TO_EXP_FM_VCO_TWO_BUTTON_PARAM].getValue() > 0.f ?
    (inputs[VCO_ONE_MOD_AMOUNT_INPUT].getVoltage() / 10.f) + params[VCO_ONE_MOD_AMOUNT_KNOB_PARAM].getValue() : 0.f;
  couplingState.vco1ToVcfExpFm = params[VCO_ONE_TO_VCF_BUTTON_PARAM].getValue() > 0.f ?
    (inputs[VCO_ONE_MOD_AMOUNT_INPUT].getVoltage() / 10.f) + params[VCO_ONE_MOD_AMOUNT_KNOB_PARAM].getValue() : 0.f;
  couplingState.vco2ToVcfLinFm = (inputs[VCO_TWO_TRI_VCF_INPUT].getVoltage() / 10.f) + params[VCO_TWO_TRI_VCF_KNOB_PARAM].getValue();

  couplingState.syncHardSub = params[VCO_TWO_TO_HARD_SYNC_VCO_ONE_BUTTON_PARAM].getValue() > 0.f;
  couplingState.syncSoft = params[VCO_TWO_TO_SOFT_SYNC_VCO_ONE_BUTTON_PARAM].getValue() > 0.f;

  // TODO: treat this like other vco1 to vco2 modulations
  couplingState.vco1ToVco2WaveSelect = params[VCO_ONE_TO_WAVE_SELECT_VCO_TWO_BUTTON_PARAM].getValue() > 0.f;
