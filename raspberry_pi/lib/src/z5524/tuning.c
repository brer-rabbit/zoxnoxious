...
int tunereq_save_state(void *zcard_plugin) {
  ...
  // structure prep
  //memset(&zcard->tunables[TUNE_SSI2130_VCO].tune_points, 0, sizeof(struct tune_point) * NUM_TUNING_POINTS);
  //memset(&zcard->tunables[TUNE_AS3394_VCO].tune_points, 0, sizeof(struct tune_point) * NUM_TUNING_POINTS);
  //memset(&zcard->tunables[TUNE_AS3394_VCF].tune_points, 0, sizeof(struct tune_point) * NUM_TUNING_POINTS);
...
tune_status_t tunereq_measurement(void *zcard_plugin, struct tuning_measurement *tuning_measurement) {
  ...
  switch (zcard->tune_target) {
  case TUNE_SSI2130_VCO:
    tp = &zcard->tunables[TUNE_SSI2130_VCO].tune_points[ zcard->tuning_index ];
    ...
    break;
  case TUNE_AS3394_VCO:
    tp = &zcard->tunables[TUNE_AS3394_VCO].tune_points[ zcard->tuning_index ];
    tp->actual_dac = tune_freq_dac_values[ zcard->tuning_index ];
    tp->frequency = tuning_measurement->frequency;
    tp->expected_dac = octave_delta(tuning_measurement->frequency, tuning_vco_initial_frequency_target) *
    ...
    break;
  case TUNE_AS3394_VCF:
    tp = &zcard->tunables[TUNE_AS3394_VCF].tune_points[ zcard->tuning_index ];
    tp->actual_dac = tune_freq_dac_values[ zcard->tuning_index ];
    tp->frequency = tuning_measurement->frequency;
...
int tunereq_restore_state(void *zcard_plugin) {
  ...
  if (zcard->tune_target == TUNE_TARGET_LENGTH) {
    create_correction_table(&zcard->tunables[TUNE_SSI2130_VCO], tuning_vco_initial_frequency_target);
    prep_correction_table_ad5328(&zcard->tunables[TUNE_SSI2130_VCO], ssi2130_vco_dac);
    create_correction_table(&zcard->tunables[TUNE_AS3394_VCO], tuning_vco_initial_frequency_target);
    prep_correction_table_ad5328(&zcard->tunables[TUNE_AS3394_VCO], as3394_vco_dac);
    create_correction_table(&zcard->tunables[TUNE_AS3394_VCF], tuning_vcf_initial_frequency_target);
    prep_correction_table_ad5328(&zcard->tunables[TUNE_AS3394_VCF], as3394_vcf_dac);
  }
  else {
    return TUNE_COMPLETE_FAILED;
  }
